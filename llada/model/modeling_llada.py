...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            num_kv_heads = k.size(1) # 32
            num_q_heads = q.size(1) # 32
            if num_q_heads != num_kv_heads: # does not go in!
    ...
    def attention(
        ...
        B, T, C = q.size()  # batch size, sequence length, d_model (1, 119, 4096). q, k, v are all the same
        dtype = k.dtype
        ...
        # shape: (B, nh, T, hs)
        # self.config.n_heads: 32, self.config.effective_n_kv_heads: 32
        nvtx.range_push("q.view")
        q = q.view(B, T, self.config.n_heads, C // self.config.n_heads).transpose(1, 2) # (1, 32, 119, 128)
        nvtx.range_pop()
        # shape: (B, n_kv_h, T, hs)
        nvtx.range_push("k.view")
        k = k.view(B, T, self.config.effective_n_kv_heads, C // self.config.n_heads).transpose(1, 2) # (1, 32, 119, 128)
        nvtx.range_pop()
        # shape: (B, n_kv_h, T, hs)
        nvtx.range_push("v.view")
        v = v.view(B, T, self.config.effective_n_kv_heads, C // self.config.n_heads).transpose(1, 2) # (1, 32, 119, 128)
        nvtx.range_pop()

        if layer_past is not None: # does not go in
        ...
        present = (k, v) if use_cache else None # does not go in
        query_len, key_len = q.shape[-2], k.shape[-2]  # could be different if layer_past not None. 119, 119 for now

        if self.config.rope: # goes here!
        ...
        if attention_bias is not None: # does not go in
        ...
        # shape: (B, nh, T, hs)
        nvtx.range_push("LLaDABlock._scaled_dot_product_attention")
        att = self._scaled_dot_product_attention( # LLaDABlock._scaled_dot_product_attention, att: (1, 32, 119, 128)
            q,
            k,
            v,
            attn_mask=None,
        ...
        nvtx.range_push("att.transpose(1, 2).contiguous().view")
        att = att.transpose(1, 2).contiguous().view(B, T, C) # (1, 119, 4096)
        nvtx.range_pop()

        # Apply output projection.
        # return self.attn_out(att), present # self.attn_out: Linear(4096, 4096), present: None
        nvtx.range_push("attn_out")
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        nvtx.range_push("attn_norm")
        x_normed = self.attn_norm(x) # x:torch.Size([1, 119, 4096])
        nvtx.range_pop()
        nvtx.range_push("q_proj")
        q = self.q_proj(x_normed) # q:torch.Size([1, 119, 4096]), Linear(in_features=4096, out_features=4096, bias=False)
        nvtx.range_pop()
        nvtx.range_push("k_proj")
        k = self.k_proj(x_normed) # k:torch.Size([1, 119, 4096]), Linear(in_features=4096, out_features=4096, bias=False)
        nvtx.range_pop()
        nvtx.range_push("v_proj")
        v = self.v_proj(x_normed) # v:torch.Size([1, 119, 4096]), Linear(in_features=4096, out_features=4096, bias=False)
        nvtx.range_pop()
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            nvtx.range_push("LLaDABlock.attention")
            att, cache = self.attention(q, k, v, attention_bias, # LLaDABlock.attention, att: (1, 119, 4096), cache: None
        ...
        x = x + self.dropout(att) # (1, 119, 4096)
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            nvtx.range_push("ff_norm")
            x = self.ff_norm(x) # goes here!
            nvtx.range_pop()
        nvtx.range_push("ff_proj, up_proj")
        x, x_up = self.ff_proj(x), self.up_proj(x) # self.ff_proj: Linear(4096, 12288), self.up_proj: Linear(4096, 12288), x: (1, 119, 12288), x_up: (1, 119, 12288)
        nvtx.range_pop()
        if self._activation_checkpoint_fn is not None:
            x = self._activation_checkpoint_fn(self.act, x)  # type: ignore
        else:
            nvtx.range_push("act")
            x = self.act(x) # goes here!, SiLU
            nvtx.range_pop()
        nvtx.range_push("x * x_up")
        x = x * x_up # new add
        nvtx.range_pop()
        nvtx.range_push("ff_out")
        x = self.ff_out(x) # Linear(12288, 4096), x: (1, 119, 4096)
        nvtx.range_pop()
        x = self.dropout(x)

        nvtx.range_push("og_x + x")
        x = og_x + x
        nvtx.range_pop()

        return x, cache # cache is None
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        batch_size, seq_len = input_ids.size() if input_embeddings is None else input_embeddings.size()[:2] # (1, 119) for the first block
        ...
        if self.config.input_emb_norm: # False
            x = x * (self.config.d_model**0.5)

        if not (self.config.alibi or self.config.rope): # False, True, so doesn't go in
        ...
        # shape: (batch_size, seq_len, d_model)
        nvtx.range_push("emb_drop")
        x = self.transformer.emb_drop(x)  # emb_drop: Dropout(p=0.0, inplace=False)
        nvtx.range_pop()
        ...
        for block_idx, block in enumerate(self.transformer.blocks): # self.transformer.blocks: 32 x LLaDALlamaBlock
            ...
            nvtx.range_push("LLaDALlamaBlock/forward" + str(block_idx)+ ", x:" + str(x.shape[0]) + "," + str(x.shape[1]) + "," + str(x.shape[2]))
            x, cache = block(x, # LLaDALlamaBlock, x: (1, 119, 4096)
            ...
            nvtx.range_pop() # pop block_idx

            if attn_key_values is not None: # does not go in!
        ...
        nvtx.range_push("ln_f")
        x = self.transformer.ln_f(x)  # RMSLayerNorm
        nvtx.range_pop()
        if output_hidden_states:
        ...
        if self.config.weight_tying:
            ...
            nvtx.range_push("ff_out")
            logits = self.transformer.ff_out(x)  # goes here!, Linear(4096, 126464)
            nvtx.range_pop()
        if self.config.scale_logits: # does not go in!
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        nvtx.range_push("LLaDAModel/forward")
        outputs = self.model.forward( # goes to LLaDAModel/forward
            input_ids=input_ids, # (1, 119)
        ...
        nvtx.range_pop()
        logits = outputs.logits # (1, 119, 126464)
        hidden_states = outputs.hidden_states # None

        loss = None
        if labels is not None: # does not go in!
            import warnings
            warnings.warn("Note that for LLaDA, you cannot calculate the loss here.", UserWarning)
        if not return_dict: # return_dict is True, so does not go in!
            output = (logits,) + outputs[1:]
            return (loss,) + output if loss is not None else output

        return CausalLMOutputWithPast(
            logits=logits, # (1, 119, 126464)
            past_key_values=outputs.attn_key_values, # None
